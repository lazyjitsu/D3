function show() {
    ...
    var loadedData;
	console.log("hi marko im running");
    d3.csv('./data/businessFiltered.csv',
        function(row) {
            // shortening the labels so they look better in the donut chart!
            switch (row.yearsInBusiness) {
                case "001" : row.yearsInBusinessLabel = "All"; break;
                case "311" : row.yearsInBusinessLabel = "Uunder 2 years"; break;
...
// setup the donut!  d3.arc() is not an svg element you can put on a page but its a generator ! A generator is a function that,
// when called, returns a path string that we can later add to a path element.
    var arc = d3.arc()
...
// this whole pieContainer part could have been added to the d3.arc() code above (fyi)
    pieContainer.append('path')
        .attr("class", 'backgroundArc')
        .attr("d", arc({startAngle: 0, endAngle: 2 * Math.PI})); // <-- we use this to create the full donut!!
