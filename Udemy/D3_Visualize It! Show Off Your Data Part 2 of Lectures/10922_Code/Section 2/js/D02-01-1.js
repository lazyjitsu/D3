function show() {
    ...
    d3.csv('./data/businessFiltered.csv',
        ...
        function (data) {
            loadedData = data; // this is how we get the data array out of the callback and back to the main program!
            updateCircle();
        });
    ...
    var colors = function(i) {
        // interPolate value of 0 i.e. if i/6 = 0; d3.interPolate(0) = value at the left
        // interPolate value of .5 i.e. d3.interPolate(0.5) - value at the center
        // interPolate value of 1 i.e. d3.interPolate(1) - value at the right
        // see d3's interPolate standard colors etc..
        return d3.interpolateReds(i/6);
    }
    /* For this we use the D-3 arc function the D-3 arc function isn't an SVG element
    we can add to the page but it is a generator. Generator is a function that when called, returns a path
    string that we can then add to a path element. */
    var arc = d3.arc()
    ...
    function update() {
        ...
        updateCircle(show); // call updateCircle with the value from the selected drop down
    }
